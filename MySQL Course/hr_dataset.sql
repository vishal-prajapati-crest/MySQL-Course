...
select concat(FIRST_NAME,' ',LAST_NAME) as name, department_id from employees where department_id IN (30,100) order by department_id ASC;
...
select concat(FIRST_NAME,' ',LAST_NAME) as name, department_id from employees where (salary NOT BETWEEN 10000 AND 15000) AND department_id IN (30,100);
...
WHERE employees.salary = (select MIN_SALARY from jobs where jobs.job_id = employees.job_id);
